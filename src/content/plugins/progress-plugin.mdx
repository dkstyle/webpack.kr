...
- `progressBar` (`boolean | 'auto' | object = false`) <Badge text="5.107.0+" />: 단순한 percentage 메세지 대신 터미널에 progress bar를 렌더링합니다. 기본 handler를 사용할 때만 적용됩니다. 즉 사용자 지정 `handler`를 제공하지 않았을 때만 동작합니다. 기본값으로 활성화하려면 `true`를 전달하고, 대화형 터미널(TTY)에서만 활성화하려면 `'auto'` <Badge text="5.109.0+" />를 전달하거나, 아래 속성을 가진 object를 전달할 수 있습니다.
...
- `estimatedTime` (`boolean = false`) <Badge text="5.109.0+" />: build progress를 기준으로 남은 예상 시간을 보여줍니다.
- `phaseTimings` (`boolean = false`) <Badge text="5.109.0+" />: build가 완료되면 각 build phase의 timing breakdown을 보여줍니다.
