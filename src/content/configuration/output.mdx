...
CSS 파일의 head 태그에 생성된 메타데이터를 압축할지 여부를 결정하는 옵션입니다. 이 옵션의 기본값은 각각 `프로덕션`에서는 `true`이고 `개발`[모드](/configuration/mode/)에서는 `false`입니다.
...
| [ext]      | `.`으로 시작하는 확장자 ([output.filename](#outputfilename)에 사용할 수 없음) |
