...
- `node_module`: `output.module`이 활성화되면 CommonJS 모듈의 `__filename`을 `fileURLToPath(import.meta.url)`로 바꿉니다.
- `'eval-only'`
...
- `node_module`: `node_module`: `output.module`이 활성화된 경우 CommonJS 모듈의 `__dirname`을 `fileURLToPath(import.meta.url + "/..")`로 바꿉니다.
- `'eval-only'`
