...
description: Logger Interface API의 기능과 사용 방법을 설명합니다.
sort: 6
contributors:
  - EugeneHlushko
  - wizardofhogwarts
...
export default function (source) {
  // webpack 로더에서 `this.getLogger`를 사용하여 로거를 가져올 수 있습니다.
...
플러그인/로깅이 컴파일과 관련된 경우 `compilation.getLogger`를 사용하는 것이 좋으며 통계 안에 저장됩니다. 컴파일 주기 밖에서 발생하는 로깅의 경우에는 대신 `compiler.getInfrastructureLogger`를 사용하세요.
...
- `import logging from 'webpack/lib/logging/runtime'`: 런타임에서 로거를 사용하려면 webpack에서 직접 require 해야 합니다.
