...
description: Module Methods API의 기능과 사용 방법을 설명합니다.
group: Modules
sort: 7
contributors:
  - skipjack
...
완전히 동적인 `import(foo)`는 오류가 아닙니다 — webpack은 여전히 이를 위해 [컨텍스트 모듈](/guides/dependency-management/)을 생성하지만 비어있는 상태입니다. 빌드 시 `Critical dependency: the request of a dependency is an expression`을 보고하며, 해당 호출에 대해 아무것도 번들하지 않고 반환된 프로미스는 런타임에 거부됩니다. 파서가 어떤 파일이 요청될 수 있는지 알 수 없고, 표현식이 해석될 수 있는 모든 파일을 포함하는 것은 거의 원하는 바가 아니기 때문에 아무것도 포함되지 않습니다.

원하는 경우, [모듈 컨텍스트](/configuration/module/#module-contexts) 옵션을 사용하여 활성화할 수 있습니다 — `exprContextRequest`는 탐색할 디렉터리, `exprContextRegExp`는 그 안의 파일 선택, `exprContextRecursive`는 서브디렉터리 포함 여부를 결정하며, `exprContextCritical`은 경고를 제어합니다.
...
W> 이 방식으로 매칭된 모든 모듈은 번들에 포함되므로, 출력이 크게 증가할 수 있습니다. 위 예시처럼 요청에 정적 접두사를 유지하는 것을 권장합니다. 분석 불가능한 `require(expr)`에 대한 동등한 옵션은 `unknownContextRequest`, `unknownContextRegExp`, `unknownContextRecursive`, `unknownContextCritical`로 명명됩니다.
...
T> 한 줄 주석(`//`)도 지원하지만 JSDoc 주석(`/** */`)은 지원하지 않습니다.
...
T> 같은 주석을 webpack의 [내장 CSS 지원](/guides/native-css/)이 처리하는 CSS에서도 사용할 수 있습니다. `@import` 또는 `url()` 앞에 `/* webpackIgnore: true */`를 추가하면 작성된 내용을 그대로 유지합니다.

**HTML Usage**

<Badge text="5.107.0+" />

[`experiments.html`](/configuration/experiments/#experimentshtml)을 활성화하면 태그 바로 앞에 `webpackIgnore` HTML 주석을 추가하여 해당 태그의 `src`, `href`, `srcset` 및 유사한 속성의 URL 리졸브를 건너뛸 수 있습니다. 내보낸 HTML에서도 태그를 변경하지 않습니다.
...
magic comment 값은 JS 및 CSS parser가 사용하는 것과 동일한 context로 파싱되며, boolean이 아닌 값은 `UnsupportedFeatureWarning`을 발생시킵니다.
...
특정 동적 import에 대한 [`fetchPriority`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLScriptElement/fetchPriority)를 설정합니다. [`module.parser.javascript.dynamicImportFetchPriority`](/configuration/module/#moduleparserjavascriptdynamicimportfetchpriority) 옵션을 사용하여 모든 동적 import에 전역 기본값을 설정할 수도 있습니다.
...
- `'eager'`: 추가 청크를 생성하지 않습니다. 모든 모듈은 현재 청크에 포함되며 추가 네트워크 요청은 발생하지 않습니다. 이미 해석된 `Promise`가 반환됩니다. 정적 가져오기와 달리 모듈은 `import()`가 호출될 때까지 실행되지 않습니다. 청크가 생성되지 않으므로 `webpackChunkName`은 이 모드에서 효과가 없습니다 — 모듈을 별도 청크에 넣으려면 `'lazy'` 또는 `'lazy-once'`를 사용하세요.
...
향후 탐색에 리소스가 필요할 수 있음을 브라우저에 알립니다. [webpackPrefetch 작동 방식](/guides/code-splitting/#prefetchingpreloading-modules)에 대한 자세한 내용은 가이드를 확인하세요.

##### webpackPreload

현재 탐색 중에 리소스가 필요할 수 있음을 브라우저에 알립니다. [webpackPreload 작동 방식](/guides/code-splitting/#prefetchingpreloading-modules)에 대한 자세한 내용은 가이드를 확인하세요.
...
W> `webpackExports`는 destructuring 할당과 함께 사용할 수 없습니다.
...
```js
// file.js에서
// ESM에서는 수동 캐시 조작이 지원되지 않습니다.
// Webpack은 모듈 캐싱을 내부적으로 처리합니다.
```
...
W> 이러한 구문은 구식입니다. 최신 애플리케이션에는 ES6 모듈 사용을 적극 권장합니다.

비동기 모듈 정의(AMD)는 모듈을 작성하고 로드하기 위한 인터페이스를 정의하는 JavaScript 사양입니다. Webpack은 다음과 같은 AMD 메서드를 지원합니다.
...
W> `name` 인수는 모듈 ID에 영향을 주지 않습니다. webpack은 이를 로컬 모듈로 등록하므로, 같은 파일의 다른 `define` 또는 `require` 호출이 이를 참조할 수 있습니다.
...
`directory`에 대한 경로, `includeSubdirs` 옵션, 포함된 모듈을 좀 더 세밀하게 제어하기 위한 `filter`, 로드 작동 방식을 정의하는 `mode`를 사용하여 전체 의존성 그룹을 지정합니다. 그러면 기본 모듈은 나중에 해석할 수 있습니다.
...
W> `require.include()`는 기본적으로 `RequireIncludeDeprecationWarning` ("require.include() is deprecated and will be removed soon.")을 내보냅니다. 경고를 억제하려면 [`module.parser.javascript.requireInclude`](/configuration/module/#moduleparserjavascript)를 `true`로 설정하거나, 기능을 비활성화하려면 `false`로 설정하세요.
