...
{/* cspell:ignore cosnole — the guide deliberately misspells console to demonstrate a source map */}
...
`webpack-dev-server`는 간단한 웹 서버와 live reloading 기능을 제공합니다. 설정해 보겠습니다.
...
T> `optimization.runtimeChunk: 'single'`이 추가된 이유는 이 예제에서 단일 HTML 페이지에 하나 이상의 엔트리 포인트가 있기 때문입니다. 이것이 없으면 [여기에](https://bundlers.tooling.report/code-splitting/multi-entry/) 설명된 문제가 발생할 수 있습니다. 자세한 내용은 [코드 스플리팅](/guides/code-splitting/) 내용을 참고하세요.
...
W> webpack-dev-server는 컴파일 후 출력 파일을 작성하지 않습니다. 대신 번들 파일을 메모리에 보관하고 서버의 루트 경로에 마운트 된 실제 파일인 것처럼 제공합니다. 페이지가 다른 경로에서 번들 파일을 찾을 것으로 예상하는 경우 개발 서버 설정에서 [`devMiddleware.publicPath`](/configuration/dev-server/#devserverdevmiddleware) 옵션을 사용하여 변경할 수 있습니다.
