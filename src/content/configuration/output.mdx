...
- `base64url` - URL 안전 Base64 인코딩(RFC 4648), 파일 이름에 권장
- `base62` - Base62 인코딩(0-9, A-Z, a-z)
- `base58` - Base58 인코딩(비트코인 알파벳)
- `base52` - Base52 인코딩(A-Z, a-z)
- `base49` - Base49 인코딩
- `base36` - Base36 인코딩(0-9, A-Z)
- `base32` - Base32 인코딩(RFC 4648)
- `base25` - Base25 인코딩
...
[추출된 HTML 모듈](/configuration/module/#modulegeneratorhtmlextract)(초기 청크)에 의해 생성되는 HTML 파일의 파일명 템플릿입니다. [`output.filename`](#outputfilename)과 같은 방식으로 작동하며 동일한 [템플릿 문자열](#template-strings)을 지원합니다. 설정하지 않으면 `output.filename`에서 `.js` 확장자를 `.html`로 교체하여 파생됩니다(`output.filename`이 함수인 경우 `[name].html`로 폴백). 이는 `cssFilename`이 `filename`에서 파생되는 방식을 미러링합니다.
...
```js
globalThis.clientContainer.define(/* define args */); // 또는 'amd-require' window['clientContainer'].require(/*require args*/);
```
...
다음 예에서는 `_entry_return_`을 사용하여 엔트리 포인트에서 반환한 값을 나타냅니다.
...
하지만 이 기능은 아직 실험 단계이며 완전히 지원되지 않으므로 미리 [experiments.outputModule](/configuration/experiments/)을 활성화해야 합니다.
...
W> `output.module`은 실험적 기능이며 [`experiments.outputModule`](/configuration/experiments/#experiments)을 `true`로 설정해야만 활성화 할 수 있습니다. [알려진 문제 중 하나](https://github.com/webpack/webpack/issues/11277#issuecomment-992565287)는 이러한 라이브러리가 webpack4 기반(및 다른 가능성도 있음) 응용 프로그램에서 사용할 수 없다는 것입니다.
