...
description: SplitChunksPlugin의 기능과 사용 방법을 설명합니다.
group: webpack
contributors:
  - sokra
  - jeremenichelli
...
- 새 청크가 20kb보다 큰 경우(min+gz 이전에; `production` 모드 이외에서는 10kb)
- 요청 시 청크를 로드할 때 최대 병렬 요청 수가 30개 이하일 경우(`production` 모드 이외에서는 무제한)
- 초기 페이지 로드 시 최대 병렬 요청 수가 30개 이하일 경우(`production` 모드 이외에서는 무제한)

마지막 두 가지 조건을 충족하려고 할 때 더 큰 청크가 선호됩니다.

T> 기본 요청 제한은 경험적으로 정한 안전장치이며, HTTP/1에만 해당하는 권장 사항이 아닙니다. 브라우저와 HTTP/2 같은 전송 방식은 많은 병렬 요청을 처리할 수 있지만, 청크가 늘어나면 스케줄링, 헤더, 캐시 조회, 파싱, 평가에 드는 오버헤드도 증가합니다. 애플리케이션에 더 세분화된 청크가 유리하다면 [`splitChunks.maxAsyncRequests`](#splitchunksmaxasyncrequests)와 [`splitChunks.maxInitialRequests`](#splitchunksmaxinitialrequests)를 조정한 다음, 사용자에게 미치는 결과를 측정하세요.
...
이 설정 객체는 `production` [`mode`](/configuration/mode/)에서 `SplitChunksPlugin`의 기본 동작을 나타냅니다. 주석은 다른 모드에서 사용되는 값을 나타냅니다.
...
export default {
  ...
  optimization: {
    splitChunks: {
      chunks: "async",
      minSize: 20000, // 10000 in other modes
      minRemainingSize: 20000, // equals minSize; 0 in development mode
      minChunks: 1,
      maxAsyncRequests: 30, // Infinity in other modes
      maxInitialRequests: 30, // Infinity in other modes
      enforceSizeThreshold: 50000, // 30000 in other modes
      cacheGroups: {
        defaultVendors: {
          test: /[\\/]node_modules[\\/]/,
          priority: -10,
...
최적화를 위해 선택될 청크를 나타냅니다. 문자열이 제공될 때 유효한 값은 `all`, `async` 및 `initial`입니다.

- `'initial'`은 애플리케이션 코드가 실행되기 전에 브라우저가 미리 로드하는 청크, 즉 [엔트리 포인트](/concepts/entry-points/)에 속하는 청크(런타임 및 정적으로 import하는 모든 것 포함)를 선택합니다.
- `'async'`는 온디맨드로 로드되는 청크, 즉 [`import()`](/api/module-methods/#import-1)와 다른 비동기 디펜던시로 생성된 청크를 선택합니다.
- `'all'`은 둘 다 선택합니다.

청크는 속한 청크 그룹 중 하나 이상이 페이지와 함께 로드될 때 initial로 간주됩니다. 따라서 엔트리 포인트의 일부이면서 다른 곳에서 `import()`로 가져오는 청크는 async가 아닌 initial로 처리됩니다. `all`을 제공하는 것은 비동기 청크와 비비동기 청크 간에도 청크를 공유할 수 있다는 것을 의미하기 때문에 특히 강력할 수 있습니다.
...
T> 이 설정을 [`output.html`](/configuration/output/#outputhtml)과 함께 사용하세요. webpack이 단일 페이지 또는 엔트리별 페이지를 생성하고, 생성된 모든 vendor 청크를 자동으로 주입합니다. [Native HTML](/guides/native-html/)을 참고하세요.
...
`splitChunks.maxAsyncRequests`의 기본값은 [`mode`](/configuration/mode/)에 따라 달라집니다.
...
`splitChunks.maxInitialRequests`의 기본값은 [`mode`](/configuration/mode/)에 따라 달라집니다.
...
크기에 숫자를 사용할 때 적용되는 크기 유형을 설정합니다. 내장 CSS 지원([`experiments.css`](/configuration/experiments/#experimentscss))이 비활성화되면 기본값은 `['javascript', 'unknown']`입니다.
...
`splitChunks.minSize`의 기본값은 [`mode`](/configuration/mode/)에 따라 달라집니다.
...
T> 청크가 생성되려면 `splitChunks.minSizeReduction` 과 `splitChunks.minSize`가 모두 충족되어야 합니다.
...
`splitChunks.enforceSizeThreshold`의 기본값은 [`mode`](/configuration/mode/)에 따라 달라집니다.
...
`splitChunks.minRemainingSize`의 기본값은 [`mode`](/configuration/mode/)에 따라 달라집니다.

| 모드            | 기본값                     |
| --------------- | -------------------------- |
| `"production"`  | `splitChunks.minSize`의 값 |
| `"development"` | `0`                        |
| `"none"`        | `splitChunks.minSize`의 값 |

세밀한 제어가 필요한 드문 경우를 제외하면 직접 지정할 필요가 없습니다.
...
T> `maxInitialRequest/maxAsyncRequests`는 `maxSize`보다 우선순위가 높습니다. 실제 우선순위는 `maxSize < maxInitialRequest/maxAsyncRequests < minSize`입니다.
...
함수를 명시한 경우 청크의 이름을 선택하는 데 특히 유용한 `chunk.name` 속성(여기서 `chunk`는 `chunks` 배열의 요소)을 찾을 수 있습니다.

`splitChunks.name`이 기존 청크의 이름(예: [엔트리 포인트](/configuration/entry-context/#entry))과 일치하는 경우, 선택된 모듈은 해당 청크가 모든 선택된 청크의 부모일 때만(예: [`dependOn`](/configuration/entry-context/#dependencies)을 통해) 해당 청크로 이동됩니다. 그렇지 않으면 webpack이 오류를 발생시킵니다.

T> `splitChunks.cacheGroups.{cacheGroup}.name`은 소스청크의 부모인 청크로 모듈을 이용하는 데 사용할 수 있습니다. 예를 들어 `name: "entry-name"`을 사용하여 모듈을 `entry-name` 청크로 이동합니다. 요청 시 명명된 청크를 사용할 수도 있지만 선택한 모듈이 청크 아래에서만 사용된다는 점에 주의해야 합니다.
...
모듈이 export할 파일의 이름을 수정(mangle)하고 사용하지 않는 export를 생략하고 보다 효율적인 코드를 생성하기 위해 어떤 export를 사용하는지 알아봅니다.

[`optimization.usedExports`](/configuration/optimization/#optimizationusedexports)가 `true`일 때(프로덕션 모드의 경우) 기본값은 `true`이고, 그렇지 않으면 `false`입니다.

### splitChunks.fallbackCacheGroup

`object`

어떤 캐시 그룹도 선택하지 않은 모듈에 대한 옵션으로, [`maxSize`](#splitchunksmaxsize)를 준수하기 위해 청크를 추가로 분할해야 할 때 webpack이 사용하는 그룹입니다. `chunks`, `minSize`, `maxSize`, `maxAsyncSize`, `maxInitialSize` 및 `automaticNameDelimiter`를 허용하며, 각각 `splitChunks` 수준에서 설정된 값을 기본값으로 사용합니다.
...
현재 청크에 이미 기본 번들에서 분리된 모듈이 포함되어 있으면 새로 생성되는 대신 재사용됩니다. 이것은 청크의 파일 이름에 영향을 줄 수 있습니다. 내장된 `default` 및 `defaultVendors` 캐시 그룹은 이 값을 `true`로 설정합니다.
...
캐시 그룹에 의해 선택되는 모듈을 제어합니다. 생략하면 모든 모듈이 선택됩니다. 모듈의 리소스 경로(`module.nameForCondition()`)와 대조됩니다. 문자열은 접두사로 일치하고, `RegExp`는 테스트로 일치합니다.
...
이 캐시 그룹이 생성하는 청크(initial 및 on-demand 모두)의 파일 이름 템플릿을 재정의합니다. `splitChunks.filename`으로 전역적으로 설정할 수도 있습니다.
[`output.filename`](/configuration/output/#outputfilename)에서 사용 가능한 모든 플레이스홀더는 여기에서도 사용할 수 있습니다.
...
- 조건 2: `helpers`가 20kb보다 큽니다
- 조건 3: import 호출 시 병렬 요청 수는 2입니다
- 조건 4: 초기 페이지 로드 시 요청에 영향을 주지 않습니다

`helpers`의 내용을 각 청크에 넣으면 코드가 두 번 다운로드됩니다. 별도의 청크를 사용하면 한 번만 발생합니다. 우리는 추가 요청 비용을 지불하며 이는 절충안으로 간주할 수 있습니다. 그렇기 때문에 최소 크기는 20kb입니다.
