---
title: Plugin Patterns
description: webpack에 기여할 때 필요한 Plugin Patterns 내용을 설명합니다.
sort: 5
contributors:
  - nveenjain
  - EugeneHlushko
  - benglynn
  - raj-sapalya
translators:
  - yoonminsohappy
---

플러그인은 webpack 빌드 시스템 내에서 커스터마이징을 수행할 수 있는 무한한 기회를 제공합니다. 이를 통해 커스텀 애셋 유형을 생성하고, 고유한 빌드 수정을 수행할 수 있게 하며, 심지어 미들웨어를 사용하는 동안 webpack 런타임을 향상시킬 수 있습니다. 다음은 플러그인을 작성할 때 유용하게 쓰이는 webpack의 몇 가지 기능입니다.

## Exploring assets, chunks, modules, and dependencies

컴파일이 완료된 후에는, 컴파일 내의 모든 구조를 확인할 수 있습니다.

```js
class MyPlugin {
  apply(compiler) {
    compiler.hooks.emit.tapAsync("MyPlugin", (compilation, callback) => {
      // 각 청크(빌드 출력)를 탐색합니다.
      for (const chunk of compilation.chunks) {
        // 청크 내의 각 모듈(빌드 입력)을 탐색합니다:
        for (const module of compilation.chunkGraph.getChunkModules(chunk)) {
          // 모듈에 포함된 각 소스 파일 경로를 탐색합니다:
          if (module.buildInfo && module.buildInfo.snapshot) {
            for (const fileDependency of module.buildInfo.snapshot.getFileIterable()) {
              // 이제 소스 코드 구조에 대해 많은 것을 알게 되었습니다...
              console.log(fileDependency);
            }
          }
        }

        //  청크에 의해 생성된 각 애셋 파일 이름을 탐색합니다.
        for (const filename of chunk.files) {
          // 청크에 의해 생성된 각 파일의 애셋 소스를 가져옵니다.
          const source = compilation.assets[filename].source();
        }
      }

      callback();
    });
  }
}

export default MyPlugin;
```

- `compilation.modules`: 컴파일 결과물의 모듈(빌드된 입력) 집합입니다. 각 모듈은 소스 라이브러리의 원시 파일의 빌드를 관리합니다.

W> **Deprecation warning**: 배열 함수는 계속 작동합니다.

- `module.buildInfo.snapshot.getFileIterable()`: 모듈에 포함된 소스 파일 경로의 이터러블입니다. 자바스크립트 소스 파일 자체(예: `index.js`)와 그것이 필요로 하는 모든 의존성 애셋 파일(스타일시트, 이미지 등)을 포함합니다. 의존성을 검토하면 어떤 소스 파일이 모듈에 속하는지 파악할 수 있습니다.
- `compilation.chunks`: 컴파일의 청크(빌드 출력) 집합입니다. 각 청크는 최종 렌더링된 애셋의 구성을 관리합니다.

W> **Deprecation warning**: 배열 함수는 계속 작동합니다.

- `compilation.chunkGraph.getChunkModules(chunk)`: 청크에 포함된 모듈의 배열입니다. 각 모듈의 의존성을 통해 어떤 원시 소스 파일이 청크에 공급되는지 확인할 수 있습니다.
- `chunk.files`: 청크에 의해 생성된 출력 파일 이름의 집합입니다. `compilation.assets` 테이블에서 이러한 애셋 소스를 액세스할 수 있습니다.

### Monitoring the watch graph

webpack 미들웨어를 실행하는 동안, 각 컴파일 결과물은 `fileDependencies` `Set`(감시 중인 파일 목록)을 포함하며, 컴파일러는 현재 재빌드를 트리거한 파일 목록이 담긴 `modifiedFiles` `Set`을 노출합니다(첫 번째 빌드에서는 `undefined`). 다음은 컴파일 결과물 내에서 변경된 파일을 감지하는데 매우 유용합니다:

```js
class MyPlugin {
  apply(compiler) {
    compiler.hooks.emit.tapAsync("MyPlugin", (compilation, callback) => {
      // 이전 빌드 이후 변경된 감시 파일만
      const changedFiles = compiler.modifiedFiles
        ? [...compiler.modifiedFiles].filter((file) =>
            compilation.fileDependencies.has(file),
          )
        : [];

      console.log("Changed files:", changedFiles);
      callback();
    });
  }
}

export default MyPlugin;
```

파일이 변경될 때 컴파일 트리거를 수신하도록 감시 그래프에 새로운 파일 경로를 제공할 수도 있습니다. 유효한 파일 경로를 `compilation.fileDependencies` `Set` 에 더하여 감시된 파일에 추가합니다.

T> `fileDependencies` `Set` 은 각 컴파일 결과물 내에서 다시 빌드되므로, 플러그인은 감시를 유지하기 위해 각 컴파일 결과물 자체에 감시된 의존성을 추가해야 합니다.

W> wepback 5부터, `compilation.fileDependencies` , `compilation.contextDependencies` 그리고 `compilation.missingDependencies` 는 이제 더 이상 `Sortable Set` 이 아니라 `Set` 이므로 정렬되지 않습니다.

## Changed chunks

감시 그래프와 유사하게, 컴파일 내에서 변경된 청크는 콘텐츠 해시를 추적하여 모니터링할 수 있습니다.

W> `compilation.chunks`는 배열이 아니라 `Set`입니다. `.filter()` 또는 `.map()`을 직접 호출하면 더 이상 사용되지 않는 호환성 레이어(`experiments.backCompat`, 기본적으로 활성화)를 통해서만 동작하며, 이 레이어가 비활성화되면 `TypeError`가 발생합니다.
`for...of`를 사용하거나 먼저 배열로 펼쳐서 사용하세요.

T> `chunk.hash`는 청크의 전체 해시이며, `chunk.contentHash[type]`은 소스 타입별(예: `javascript`, `css`) 콘텐츠 해시입니다.
청크의 JavaScript 콘텐츠 변경을 감지하려면 `chunk.contentHash.javascript`를 사용하는 것이 좋습니다.

```js
class MyPlugin {
  constructor() {
    this.chunkVersions = {};
  }

  apply(compiler) {
    compiler.hooks.emit.tapAsync("MyPlugin", (compilation, callback) => {
      const changedChunks = [];

      for (const chunk of compilation.chunks) {
        const key = chunk.id ?? chunk.name;
        const currentHash = chunk.contentHash.javascript;

        if (currentHash === undefined) continue;

        const oldHash = this.chunkVersions[key];
        this.chunkVersions[key] = currentHash;

        if (currentHash !== oldHash) {
          changedChunks.push(chunk);
        }
      }

      if (changedChunks.length > 0) {
        console.log(
          "Changed chunks:",
          changedChunks.map((chunk) => chunk.id),
        );
      }

      callback();
    });
  }
}

export default MyPlugin;
```

T> 추적 key로 `chunk.id ?? chunk.name`을 사용하세요. 일부 chunk에는 `id`가 있지만 `name`이 없으므로 `chunk.name`에만 의존하면 변경 사항을 놓칠 수 있습니다.
CSS 또는 기타 JavaScript가 아닌 chunk에는 `contentHash`의 `javascript` key가 없을 수 있습니다. `continue` guard는 이러한 chunk를 안전하게 건너뜁니다.
