...
| Hook                | 타입                                    | 설명                                                                                  |
| ------------------- | --------------------------------------- | ------------------------------------------------------------------------------------- |
| `done`              | `SyncHook<[MultiStats, Compiler[]]>`    | 모든 child compilation이 끝나면 한 번 호출됩니다. `MultiHook`이 아닌 집계 hook입니다. |
| `shutdown`          | `AsyncSeriesHook<[Compiler]>`           | <Badge text="5.110.0+" /> compiler 종료 시 child별로 한 번 호출됩니다.                |
| `invalid`           | `SyncHook<[string \| null, number]>`    | child에서 watch 중인 파일이 변경되면 호출됩니다.                                      |
