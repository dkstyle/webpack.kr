...
description: webpack의 Watch and WatchOptions 설정과 사용 방법을 설명합니다.
sort: 14
contributors:
  - sokra
  - skipjack
...
T> `require.context`를 사용하면 webpack이 전체 디렉터리를 감시합니다. 원치 않는 변경으로 인해 재빌드가 발생하지 않도록 파일이나 디렉터리를 무시해야 합니다.

### watchOptions.poll

`boolean = false` `number`

기본 폴링 간격을 [`5007`](https://github.com/webpack/watchpack/blob/a54bcdb95759558ca5a9fc2819c4d71b771c162f/lib/DirectoryWatcher.js#L79)로 설정하는 `true`를 전달하거나 폴링 간격을 밀리초(ms)로 지정하여 [polling](<https://en.wikipedia.org/wiki/Polling_(computer_science)>)을 사용할 수 있습니다.
...
T> 만약 감시 기능이 작동하지 않는다면, 이 옵션을 사용해 보세요. 이는 VirtualBox, WSL, Containers 또는 Docker의 NFS 및 시스템 문제해결에 도움이 될 수 있습니다. 이 경우 폴링 간격을 사용하고 `/node_modules/`와 같은 큰 폴더를 무시하여 CPU 사용량을 최소화하세요.
