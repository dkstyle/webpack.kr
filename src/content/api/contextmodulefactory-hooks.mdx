...
description: ContextModuleFactory Hooks API의 기능과 사용 방법을 설명합니다.
group: Plugins
sort: 11
contributors:
  - iguessitsokay
...
`ContextModuleFactory` 클래스는 tapable 훅의 `hooks` 객체를 노출하며 다음과 같은 생명주기 훅을 제공합니다.
...
정규 표현식으로 필터링하기 전에, 생성된 `{ context, request }` 대안 배열과 함께 디렉터리마다 한 번씩 호출됩니다. (수정될 수 있는) 배열을 반환합니다.

- 콜백 파라미터: `modules` `options`
