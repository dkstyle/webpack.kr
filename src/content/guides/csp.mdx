...
description: webpack에서 Content Security Policies을 사용하는 방법을 설명합니다.
sort: 10
contributors:
  - EugeneHlushko
  - probablyup
...
Webpack은 로드하는 모든 스크립트에 `nonce`를 추가할 수 있습니다. 기능 세트를 활성화하려면 엔트리 스크립트에 `__webpack_nonce__` 변수를 포함해야 합니다. 고유한 해시 기반 nonce가 생성되고 고유한 페이지 뷰에 대해 각각 제공됩니다. 이것이 바로 `__webpack_nonce__` 가 설정이 아닌 엔트리 파일에 지정된 이유입니다. `__webpack_nonce__`는 항상 base64로 인코딩된 문자열이어야 합니다.
