...
description: webpack에서 Environment Variables을 사용하는 방법을 설명합니다.
sort: 8
contributors:
  - simon04
  - grisanu
...
Webpack 설정에서 한 가지 변경 사항을 적용해야 합니다. 일반적으로 `export default`는 설정 객체를 가리킵니다. `env` 변수를 사용하려면 `export default`를 함수로 변환해야 합니다.
...
T> Webpack CLI는 webpack 설정 내에서 접근할 수 있는 [빌트인 환경 변수](/api/cli/#cli-environment-variables)를 제공합니다.
