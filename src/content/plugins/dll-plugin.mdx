---
title: DllPlugin
description: DllPlugin의 기능과 사용 방법을 설명합니다.
group: webpack
contributors:
  - aretecode
  - sokra
...
- `name`: 노출된 dll 함수의 이름 ([TemplatePaths](https://github.com/webpack/webpack/blob/main/lib/TemplatedPathPlugin.js): `[fullhash]`, `[chunkhash]`, `[contenthash]`, & `[name]` )
...
W> Webpack 출력에서 [코드 분할](/guides/code-splitting)을 사용하는 경우 webpack이 여러 매니페스트 파일을 생성하도록 `path`에서 [템플릿 문자열](/configuration/output/#template-strings)을 사용해야 합니다.
...
이 과정은 DLL 번들 내의 모든 파일을 해석한 후에 발생하므로, DLL 번들을 사용하는 쪽에서도 동일한 경로를 사용할 수 있어야 합니다. 예를 들어, DLL에 `lodash`와 `abc` 파일이 포함되어 있다면, 메인 번들에 해당 파일들을 포함시키는 대신 DLL 자체에서 `import _ from "lodash"`/`require('lodash')`와 `import abs from "./abc"`/`require('./abc')`를 사용하게 됩니다.
