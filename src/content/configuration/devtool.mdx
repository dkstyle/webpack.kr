---
title: Devtool
description: webpack의 Devtool 설정과 사용 방법을 설명합니다.
sort: 12
contributors:
  - sokra
  - skipjack
...
보다 세밀한 설정을 위해 [`SourceMapDevToolPlugin`](/plugins/source-map-dev-tool-plugin)을 사용하세요. 기존 소스맵을 처리하려면 [`Rule.extractSourceMap`](/configuration/module/#ruleextractsourcemap)를 참고하세요.
...
기본값은 `false`입니다. [`mode: 'development'`](/configuration/mode/)에서는 JavaScript에 대해 기본적으로 `'eval'`로 설정되며([`experiments.css`](/configuration/experiments/#experimentscss)가 활성화된 경우 CSS에 대해서는 `'source-map'` 추가), `module` 또는 `modern-module` [라이브러리 타입](/configuration/output/#outputlibrarytype)이 출력되는 경우에는 `false`로 유지됩니다.

<Badge text="5.105.0+" />

또한 다양한 자산 유형에 대해 서로 다른 소스 맵 스타일을 구성하기 위해 객체 배열을 제공할 수도 있습니다.
...
`type` 필드는 어떤 자산 유형이 devtool 값을 받아야 하는지 지정합니다.

- `"all"` - 모든 자산 유형(JavaScript 및 CSS)에 적용
- `"javascript"` - JavaScript 파일에만 적용
- `"css"` - CSS 파일에만 적용

배열을 사용하는 경우 각 항목은 순서대로 처리됩니다. 문자열 값을 제공하면 `{ type: "all", use: "your-string-value" }`로 처리됩니다.
...
T> webpack은 devtool 이름 유효성을 검사할 때 특정한 패턴을 예상합니다. 이 점을 숙지하고 devtool 문자열의 순서를 뒤섞지 않아야 합니다. 패턴은 `[inline-|hidden-|eval-][nosources-][cheap-[module-]]source-map[-debugids]`입니다.
...
`(none)` (`devtool: false`로 설정하거나 `development` 모드 외부에서 옵션 생략) - 소스맵을 내보내지 않습니다. 이것은 시작하기에 좋은 옵션입니다.
...
T> 기본 webpack `minimizer`가 재정의된 경우(예를 들어 `minimizer-webpack-plugin` 옵션을 사용자 지정한 경우), SourceMap 지원을 활성화하려면 대체 항목에 `sourceMap: true`를 설정해야 합니다.
