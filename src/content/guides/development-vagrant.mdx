...
description: webpack에서 Development - Vagrant을 사용하는 방법을 설명합니다.
sort: 11
contributors:
  - SpaceK33z
  - chrisVillanueva
...
다음으로, 프로젝트에 `webpack`, `webpack-cli`, `webpack-dev-server`를 설치하세요.
...
그리고 `index.html` 파일을 만드세요. 스크립트 태그는 번들을 가리켜야 합니다. 설정에서 `output.filename`이 지정되지 않은 경우, 기본 `main` 엔트리에 대해 `main.js`(`[name].js`)가 됩니다.
...
`--client-web-socket-url` 플래그는 스크립트가 WebSocket을 찾을 위치를 알고 있는지 확인합니다. 서버는 기본적으로 `8080` 포트를 사용하므로, 여기에서도 지정해야 합니다.

`--watch-options-poll`은 webpack이 파일의 변경을 감지할 수 있도록 합니다. 기본적으로, webpack은 파일 시스템에 의해 트리거되는 이벤트를 수신하지만, VirtualBox에는 이와 관련된 많은 문제가 있습니다.
