---
title: Concepts
description: webpack의 Concepts 개념과 동작 방식을 설명합니다.
sort: -1
contributors:
  - TheLarkInn
  - jhnns
...
- [기본 모듈 번들러 라이브 코딩](https://www.youtube.com/watch?v=Gc9-7PBqOC8)
- [기본 모듈 번들러에 대한 자세한 설명](https://github.com/ronami/minipack)
...
T> 설정 파일 없이 webpack을 실행하면 entry의 기본값은 `'./src'`이며, webpack은 `resolve.mainFiles`와 `resolve.extensions`를 통해 이를 `./src/index.js`로 해석합니다. `src/` 디렉터리가 존재하더라도 해당 파일이 없으면 webpack은 다음 오류를 발생시킵니다. <br /> **`Module not found: Error: Can't resolve './src' in '/path/to/project'`** <br /><br /> 오류에 `'./src'`가 표시되는 이유는 그것이 실제 엔트리 요청이기 때문입니다. `index.js`는 그 안에서 검색되는 기본 파일일 뿐입니다. 다른 엔트리 파일 이름을 사용하려면 위 예시와 같이 `webpack.config.js`를 추가하세요.

T> [엔트리 포인트](/concepts/entry-points) 섹션에서 자세히 알아보세요.
...
webpack은 기본적으로 JavaScript, JSON, WebAssembly 파일을 이해하며, 기본적으로 활성화된 내장 지원([Native CSS](/guides/native-css/), [Native HTML](/guides/native-html/))을 통해 CSS와 HTML도 처리합니다. **로더를** 사용하면 webpack이 다른 유형의 파일을 처리하거나, 그들을 유효한 [모듈](/concepts/modules)로 변환하여 애플리케이션에서 사용하거나 디펜던시 그래프에 추가할 수 있습니다.

W> webpack의 특정 기능 중 하나인 모든 유형의 모듈(예: `.css`)을 `import` 하는 기능은 다른 번들러나 태스크 러너에서 지원하지 않을 수 있습니다. 개발자들에게 더욱 정확한 디펜던시 그래프를 생성하는 데 도움을 주기 때문에 이러한 언어의 확장이 필요하다고 생각합니다.
...
> "Webpack 컴파일러야, `require()`나 `import` 문 안에서 '.js' 파일로 해석되는 경로를 만나면, 번들에 추가하기 전에 `babel-loader`를 **사용**해서 변환해 줘."
...
플러그인을 사용하려면 `import` 연산자를 사용하여 플러그인을 가져온 다음 `plugins` 배열에 추가해야 합니다. 대부분의 플러그인은 옵션을 통해 사용자 정의할 수 있습니다. 하나의 구성에서 여러 용도로 플러그인을 여러 번 사용할 수 있으므로 `new` 연산자를 사용하여 플러그인의 인스턴스를 생성해야 합니다.
...
위 예제에서 `ProgressPlugin`은 빌드 진행 상황의 보고 방식을 설정합니다. `src/index.html`이 엔트리이므로 webpack은 해당 파일이 참조하는 script와 style을 번들링하고 URL을 빌드된 파일 이름으로 재작성합니다. 페이지 생성에는 plugin이 필요하지 않습니다. [Native HTML](/guides/native-html/)을 참고하세요.
...
Webpack은 [ES5](https://compat-table.github.io/compat-table/es5/)가 호환되는 모든 브라우저를 지원합니다(IE8 이하는 지원되지 않습니다). Webpack은 [`import()` 및 `require.ensure()`](/guides/code-splitting/#dynamic-imports)을 위한 `Promise`를 요구합니다. 구형 브라우저를 지원하려면 이러한 표현식을 사용하기 전에 [폴리필을 로드](/guides/shimming/)해야 합니다.

## Environment

Webpack 5에는 Node.js 버전 10.13.0 이상이 필요합니다.
