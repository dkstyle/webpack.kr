...
이 옵션은 CSS 파일 내의 `url()`, `image-set()`, `src()`, `image()`와 같은 함수에서 URL 처리를 활성화하거나 비활성화합니다. 활성화하면 이러한 URL은 webpack에서 해석되고 처리됩니다.
...
T> 특정 가져오기를 필터링하려면 Webpack의 기본 제공 플러그인[IgnorePlugin](/plugins/ignore-plugin/)을 사용할 수 있습니다. `css-loader`에서 사용 가능한 [`filter` 옵션](/loaders/css-loader/#filter)은 지원되지 않습니다.
...
동적 import를 위해 전역 [fetchPriority](https://developer.mozilla.org/en-US/docs/Web/API/HTMLImageElement/fetchPriority)를 지정합니다.
