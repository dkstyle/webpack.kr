...
description: webpack에서 Authoring Libraries을 사용하는 방법을 설명합니다.
sort: 7
contributors:
  - pksjce
  - johnstew
...
npm으로 프로젝트를 초기화한 다음, `webpack`, `webpack-cli`, `lodash`를 개발 의존성으로 설치합니다.
...
처음에는 `lodash`를 라이브러리에 함께 번들할 것이므로 `devDependency`로 설치합니다. 최종 출력물에 포함되기 때문에 라이브러리 사용자는 이를 별도로 설치할 필요가 없습니다.
...
라이브러리를 번들링할 때는 source map을 생성하는 것이 좋습니다. Source map을
사용하면 라이브러리 사용자가 최소화된 번들 대신 원본 소스 코드를 디버깅할 수
있습니다. [`devtool`](/configuration/devtool/) 옵션으로 설정할 수 있습니다.
...
T> `devtool` 값으로 `'source-map'`을 사용하면 번들과 함께 별도의 `.map` 파일이
생성됩니다. 사용자가 디버깅에 활용할 수 있도록 이 `.map` 파일도 함께 배포하세요.
...
`type`을 [`'umd'`](/configuration/output/#type-umd)로 설정하여 `output.library` 옵션을 업데이트해 보겠습니다.
...
T> `library` 설정은 `entry` 설정과 연관되어 있습니다. 대부분의 라이브러리의 경우 단일 엔트리 포인트를 지정하는 것으로 충분합니다. [다중 진입 라이브러리](https://github.com/webpack/webpack/tree/master/examples/multi-part-library)도 가능하지만, [index script](https://stackoverflow.com/questions/34072598/es6-exporting-importing-in-index-file)를 통해 부분적으로 export하는 것이 더 간단합니다. 라이브러리의 `entry`로 `array`를 사용하는 것은 **권장되지 않습니다.**

## Externalize Lodash

이제 `npx webpack`을 실행하면 다소 큰 번들이 생성되는 것을 볼 수 있습니다. 파일을 살펴보면 lodash가 코드와 함께 번들되어 있습니다. `lodash`를 번들에 포함해 라이브러리가 비대해지는 것을 막기 위해 webpack이 이를 외부 모듈로 취급하도록 설정할 수 있습니다. 더 이상 번들하지 않으므로 사용자가 이를 직접 제공해야 합니다. 따라서 라이브러리 사용자의 패키지 매니저가 자동으로 설치할 수 있도록 `lodash`를 `devDependencies`에서 `dependencies` 또는 `peerDependencies`로 옮겨야 합니다.
...
W> `module` 속성은 ES2015 모듈 구문을 사용하는 스크립트를 가리켜야 하지만 아직 브라우저나 노드에서 지원하지 않는 다른 구문 기능은 없습니다. 이를 통해 webpack은 사용자가 라이브러리의 특정 부분만 소비하는 경우 모듈 구문 자체를 파싱하여 [트리 쉐이킹](https://webpack.kr/guides/tree-shaking/)을 통해 보다 가벼운 번들을 제공할 수 있습니다.
...
T> 라이브러리와 관련된 스타일시트를 노출하려면 엔트리에서 CSS를 가져오고 webpack의 [내장 CSS 지원](/guides/native-css/)이 이를 추출하도록 하세요. 내보낸 `.css` 파일의 이름은 [`output.cssFilename`](/configuration/output/#outputcssfilename)에 따라 정해지며, 사용자는 다른 스타일시트와 같은 방식으로 이를 사용하고 로드할 수 있습니다.
