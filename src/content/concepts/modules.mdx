...
[모듈형 프로그래밍(modular programmming)](https://en.wikipedia.org/wiki/Modular_programming)에서 개발자는 _모듈이라는_ 개별 기능으로 프로그램을 나눕니다.
...
- [CommonJS](https://wiki.commonjs.org/wiki/Modules/1.1) `require()` 문
...
그 밖에도 webpack은 여러 언어로 작성된 모듈과 _로더를_ 통한 다양한 전처리기를 지원합니다. _로더는_ webpack에서 네이티브가 아닌 _모듈을_ **어떻게** 처리하고 이러한 _의존성을_ _번들에_ 포함할지 정의합니다.
