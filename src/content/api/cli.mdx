...
description: Command Line Interface API의 기능과 사용 방법을 설명합니다.
sort: 1
contributors:
  - anshumanv
  - rishabh3112
...
W> `webpack-cli` v7.0.0 이상 버전을 사용하려면 Node >= v20.9.0, `webpack >= v5.101.0`, `webpack-dev-server >= v5.0.0`이 필요합니다.
...
| [`version`](#version)       | `version\|v [options]`                    | 'webpack', 'webpack-cli', 'webpack-dev-server' 및 기타 패키지의 버전 번호를 출력합니다. |
...
`create-webpack-app`을 사용하여 새 webpack 프로젝트를 초기화하는 데 사용됩니다.
...
T> `create-webpack-app`의 전체 문서는 [여기](https://github.com/webpack/webpack-cli/blob/master/packages/create-webpack-app/README.md)에서 확인할 수 있습니다.
...
T> [`webpack serve` 명령에 대한 전체 옵션 목록](https://github.com/webpack/webpack-cli/blob/master/SERVE-OPTIONS-v4.md) 및 [webpack-dev-server 관련 문서](/configuration/dev-server)를 참고하세요.
...
예를 들어, 프로젝트에서 성능 힌트를 활성화하려면 설정에서 [이 옵션](https://webpack.kr/configuration/performance/#performancehints)을 사용할 수 있습니다. 코어 플래그를 사용하면 아래와 같이 할 수 있습니다.
...
W> 기존 [entry](/configuration/entry-context/#entry) 옵션을 대체하려면 `--entry-reset` 옵션이 필요하지만, 이 옵션이 없으면 `--entry` 옵션이 기존 항목에 다른 항목을 추가합니다.

T> `webpack [command] --entry-reset [entries ...] [option]`구문을 사용합니다. 일부 옵션은 복수의 값을 허용할 수 있으므로 `webpack --target node ./entry.js`는 `target: ['node', './entry.js']`를 의미합니다.
...
> 예시 - 설정 파일 조회 순서는 webpack.config.js > .webpack/webpack.config.js > .webpack/webpackfile.js입니다.
...
각 기본 이름은 지원되는 확장자(JavaScript 및 TypeScript 변형(`.js`, `.mjs`, `.cjs`, `.ts`, `.cts`, `.mts`), `.coffee`와 같은 기타 지원 언어, 그리고 webpack-cli 7.1.0부터는 [데이터 포맷](/configuration/configuration-languages/#data-formats-json5-yaml-and-toml) `.json5`, `.yaml`/`.yml` 및 `.toml`)에 대해 해석됩니다.
...
**'webpack', 'webpack-cli', 'webpack-dev-server' 및 기타 패키지의 버전 번호를 출력합니다.**
...
`--extends`를 사용하여 기존 webpack 설정을 확장할 수 있습니다.

```bash
npx webpack --extends ./base.webpack.config.js
```

[설정 확장](/configuration/extending-configurations/)에서 자세한 내용을 읽어보세요.
...
T> 변수를 명시적으로 빈 문자열(`""`)로 설정하려면, `npx webpack --env foo=\"\"`와 같이 터미널에서 문자를 이스케이프해야 할 수 있습니다.
...
설정에 따라 `process.env.NODE_ENV`를 설정하세요.
...
설정이 함수를 내보내는 경우, 함수가 반환된 후 `--config-node-env` 값이 `mode`에 할당됩니다. 즉, `mode`는 함수 인수(`env` 및 `argv`)에서 사용할 수 없습니다. 그러나 `--config-node-env`의 값은 함수 내에서 `argv.configNodeEnv`로 접근 가능하며 그에 따라 사용할 수 있습니다.

```js
export default (env, argv) => {
  console.log(argv.configNodeEnv); // --config-node-env production이 사용되는 경우 'production'
  return {
    // 당신의 설정
  };
};
...
TypeScript에서 기본 ESM을 사용하는 경우 이와 같은 오류가 발생할 수 있습니다 (예: `package.json`의 `type: "module"`).

`webpack-cli`는 `CommonJS`와 `ESM` 형식의 설정을 모두 지원하며, 처음에는 `import()`를 사용하여 설정을 로드하려고 시도하고, 실패하면 `require()`를 사용하여 설정을 로드하려고 시도합니다.
하지만 `import()` 메서드는 `ts-node`에서 [로더 훅](https://nodejs.org/api/esm.html#esm_experimental_loaders)을 활성화하지 않으면 작동하지 않습니다(자세한 내용은 [`TypeStrong/ts-node#1007`](https://github.com/TypeStrong/ts-node/issues/1007) 참고).
...
자세한 내용은 [`TypeScript`로 webpack 설정 작성](/configuration/configuration-languages/#typescript)에 대한 문서를 참고하세요.
