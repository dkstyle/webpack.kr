...
description: webpack의 Module Resolution 개념과 동작 방식을 설명합니다.
sort: 7
contributors:
  - pksjce
  - pastelsky
...
이 경우 `import` 또는 `require`가 발생하는 소스 파일의 디렉터리를 컨텍스트 디렉터리로 간주합니다. `import/require`에 지정된 상대 경로는 이 컨텍스트 경로에 결합되어 모듈에 대한 절대 경로를 생성합니다.
