---
title: Loaders
description: webpack의 Loaders 개념과 동작 방식을 설명합니다.
sort: 3
contributors:
  - manekinekko
  - evenstensberg
...
예를 들어 loader를 사용해 Sass를 CSS로 컴파일하거나 TypeScript를 JavaScript로 변환할 수 있습니다. 먼저 필요한 loader를 설치합니다.

```bash
npm install --save-dev sass-loader ts-loader
```

그런 다음 webpack이 모든 `.scss` 파일에 [`sass-loader`](/loaders/sass-loader)를, 모든 `.ts` 파일에 [`ts-loader`](https://github.com/TypeStrong/ts-loader)를 사용하도록 설정합니다.
...
T> 일반 `.css` 파일에는 loader가 필요하지 않습니다. webpack은 CSS를 직접 파싱, 추출, 최소화하는 실험적인 내장 기능을 제공합니다. [`experiments.css`](/configuration/experiments/#experimentscss)와 [Native CSS](/guides/native-css/)를 참고하세요. preprocessor에는 계속 해당 loader가 필요하며 `type: 'css/auto'`는 loader 출력을 webpack CSS 파이프라인에 전달합니다.
...
Loader는 오른쪽에서 왼쪽(또는 아래에서 위)으로 평가 및 실행됩니다. 아래 예제에서는 sass-loader로 시작하여 postcss-loader로 끝납니다. 자세한 실행 순서는 ["Loader Features"](/concepts/loaders/#loader-features)를 참고하세요.
...
T> `loader1!loader2!./file` 구문은 예시일 뿐입니다. 대부분의 프로젝트에서는 `module.rules` 파일을 통해 로더를 구성하고, 로더의 부가적인 효과를 위해 스타일을 임포트하는 것이 좋습니다(예: `import "./styles.css"`).
...
- 로더는 연결할 수 있습니다. 체인의 각 로더는 처리된 리소스에 변환을 적용합니다. 체인은 역순으로 실행됩니다. 첫 번째 로더는 결과(변환이 적용된 리소스)를 다음 로더로 전달하며, 이후 로더들도 마찬가지입니다. 마지막으로 webpack은 체인의 마지막 로더가 모듈의 `type`에 맞는 콘텐츠를 반환할 것을 기대합니다. `javascript/*` 모듈에는 JavaScript, `css/*`에는 CSS, `json`에는 JSON, `asset/*`에는 raw 콘텐츠가 해당됩니다.
- 로더는 동기식 또는 비동기식일 수 있습니다.
- 로더는 Node.js에서 실행되며 Node.js에서 가능한 모든 작업을 수행할 수 있습니다.
- 로더는 `options` 객체로 구성할 수 있습니다. (`query` 파라미터를 사용하여 옵션을 설정하는 방법은 여전히 지원되지만 더 이상 사용되지 않습니다)
- 일반 모듈은 `loader` 필드가 있는 `package.json`을 통해 일반 `main` 외에도 로더를 내보낼 수 있습니다.
- 플러그인은 로더에 더 많은 기능을 제공할 수 있습니다.
