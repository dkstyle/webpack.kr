...
description: ContextReplacementPlugin의 기능과 사용 방법을 설명합니다.
group: webpack
contributors:
  - simon04
  - byzyk
...
*Context*는 `require('./locale/' + name + '.json')`와 같은 `import()` 또는 `require()`의 동적 표현식을 의미합니다. 이러한 표현식을 만나면 webpack은 디렉터리(`./locale/'`)와 정규 표현식(`/^.*\.json$/`)을 추론합니다. 컴파일 시점에 `name`을 알 수 없으므로 webpack은 모든 파일을 모듈로 번들에 포함합니다.
...
리소스(디렉터리)가 `resourceRegExp`와 일치하면, 플러그인은 기본 리소스, 재귀 플래그 또는 생성된 정규식을 각각 `newContentResource`, `newContentRecursive` 또는 `newContentRegExp`로 바꿉니다. `newContentResource`가 상대적인 경우 이전 리소스를 기준으로 해결됩니다.
