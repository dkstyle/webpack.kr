...
T> Webpack은 5.107.0부터 HTML을 내장 지원합니다. [`experiments.html`](/configuration/experiments/#experimentshtml)과 [`output.html`](/configuration/output/#outputhtml)은 별도의 플러그인을 설치하지 않아도 이 플러그인이 제공하는 기능 대부분을 지원합니다. 기능 범위와 마이그레이션 방법은 [Native HTML](/guides/native-html/) 가이드를 참고하세요.
