...
- <Badge text="5.87.0+" /> `package.json` 파일의 스타일별 필드 해석 webpack은
  `package.json` 파일에서 `style` 필드를 찾고 CSS 파일 내부에 import를 위해
  존재하는 경우 이 파일을 사용합니다.
...
`topLevelAwait` 옵션은 `await`가 최상위 수준에서 사용될 때 모듈을 `async` 모듈로 변환합니다. Webpack 버전 `5.83.0`부터는 이 기능이 기본적으로 활성화됩니다. 그러나 그 이전 버전에서는 'experiments.topLevelAwait'를 'true'로 설정하여 활성화할 수 있습니다.
