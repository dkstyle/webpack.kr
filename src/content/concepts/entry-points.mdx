---
title: Entry Points
description: webpack의 Entry Points 개념과 동작 방식을 설명합니다.
sort: 1
contributors:
  - TheLarkInn
  - chrisVillanueva
...
T> 설정 파일 없이 webpack을 실행하면 entry의 기본값은 `'./src'`이며, webpack은 `resolve.mainFiles`와 `resolve.extensions`를 통해 이를 `./src/index.js`로 해석합니다. `src/` 디렉터리가 존재하더라도 해당 파일이 없으면 webpack은 다음 오류를 발생시킵니다. <br /> **`Module not found: Error: Can't resolve './src' in '/path/to/project'`** <br /><br /> 오류에 `'./src'`가 표시되는 이유는 그것이 실제 엔트리 요청이기 때문입니다. `index.js`는 그 안에서 검색되는 기본 파일일 뿐입니다. 다른 엔트리 파일 이름을 사용하려면 아래 섹션처럼 `entry` 옵션을 설정하세요.
...
export default {
  output: {
    filename: "[name].bundle.js", // 예: main.bundle.js, vendor.bundle.js
  },
};
```
