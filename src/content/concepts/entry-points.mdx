...
T> 설정 파일 없이 webpack을 실행하면 entry의 기본값은 `'./src/index.js'`입니다. `src/` 디렉터리가 존재하더라도 이 파일이 없으면 webpack은 다음 오류를 표시합니다. <br /> **`ERROR in Entry module not found: Error: Can't resolve './src'`** <br /><br /> webpack이 먼저 디렉터리를 resolve한 다음 그 안에서 `index.js`를 찾지 못하기 때문에 오류에는 `'./src/index.js'` 대신 `'./src'`가 표시됩니다. 다른 entry 파일 이름을 사용하려면 아래 섹션처럼 `entry` 옵션을 설정하세요.
