---
title: DevServer
description: webpack의 DevServer 설정과 사용 방법을 설명합니다.
sort: 11
contributors:
  - sokra
  - skipjack
...
이 페이지는 webpack-dev-server(약칭: dev-server)<Badge text="version >= 5.0.0" />의 동작에 영향을 미치는 옵션에 관해 설명합니다. `v4`에서 `v5`로의 마이그레이션 가이드는 [여기](https://github.com/webpack/webpack-dev-server/blob/master/migration-v5.md)를 참고하세요.

W> `webpack-dev-server v5.0.0+`에는 `node >= v18.12.0`, `webpack >= v5.0.0` 및 `webpack-cli >= v4.7.0`이 필요하므로 최신 버전을 사용하는 것이 좋습니다.
...
W> [여러 설정을 내보낼 때](/configuration/configuration-types/#exporting-multiple-configurations) `devServer` 옵션은 `devServer`를 정의한 첫 번째 설정에서 가져옵니다. 없는 경우 웹 유사 타겟(`web` 또는 `universal` 플랫폼, `web`, `webworker`, `electron-preload`, `electron-renderer`, `node-webkit` 및 설정되지 않은 `target` 포함)을 가진 첫 번째 설정에서, 그렇지 않으면 첫 번째 설정에서 가져옵니다. 이 옵션은 배열의 모든 설정에 사용됩니다.

T> 문제가 있는 경우, `/webpack-dev-server` 경로로 이동하면 파일이 제공되는 위치가 표시됩니다. 예를 들면, `http://localhost:9000/webpack-dev-server` 처럼 표시됩니다.

T> 번들을 수동으로 다시 컴파일하려는 경우, `/webpack-dev-server/invalidate` 경로로 이동하면 번들의 현재 컴파일이 [무효화](/api/node/#invalidate-watching) 되고 [webpack-dev-middleware](https://github.com/webpack/webpack-dev-middleware#invalidate)를 통해 다시 컴파일됩니다. 설정에 따라, URL은 `http://localhost:9000/webpack-dev-server/invalidate`처럼 보일 수 있습니다.
...
예를 들어, [`AbortController.abort()`](https://developer.mozilla.org/en-US/docs/Web/API/AbortController/abort)에 의해 발생한 오류를 무시하려면 다음을 수행하세요.
...
W> 이 함수는 설정 파일 내의 외부 범위에서 선언된 변수에 액세스할 수 없습니다.

### progress

`boolean` `'linear' | 'circular'`

브라우저에서 컴파일 진행률을 백분율로 출력합니다. `'linear'`과 `'circular'`는 추가로 시각적 표시기를 표시합니다.
...
클라이언트 재연결을 시도해야 하는 횟수를 dev-server에 알려줍니다. 기본값은 10입니다. `true`라면 무제한 재연결을 시도하고, `false`는 재연결을 비활성화합니다.
...
W> `bypass` 옵션은 `router` 및 `context` 옵션을 위해 프록시에서 더 이상 사용되지 않습니다. [여기에서 자세히 알아보세요](https://github.com/chimurai/http-proxy-middleware/tree/v2.0.6#http-proxy-middleware-options).
...
W> webpack-dev-server v6부터 `'spdy'`는 더 이상 내장 서버 타입이 아닙니다. `spdy` 패키지가 설치된 경우에만 커스텀 서버 문자열로 작동하며, CLI는 이를 허용하지 않고 [Node 15.0.0 이상에서 spdy에 문제가 발생합니다](https://github.com/spdy-http2/node-spdy/issues/380). Node의 내장 `http2` 모듈(HTTP/1 폴백 포함)을 사용하는 `'http2'`를 사용하세요.
