---
title: To v5 from v4
description: To v5 from v4 마이그레이션 방법을 설명합니다.
sort: 1
contributors:
  - sokra
  - salemhilal
...
webpack v4에서 v5로 업그레이드하는 것을 지원하기 위해 [Codemod](https://github.com/codemod-com/codemod)는 대부분의 마이그레이션 프로세스를 자동화하는 데 도움이 되는 오픈 소스 커뮤니티 codemod를 제공합니다.

이것들은 공식 webpack codemods가 아니며, 마이그레이션을 간소화하는 것을 목표로 하지만 모든 경우를 포괄하지는 않을 수 있습니다. 업그레이드를 완전히 완료하려면 추가 수동 단계를 수행해야 할 수도 있습니다.

[webpack v5 마이그레이션 codemods](https://go.codemod.com/webpack-v5-recipe)를 실행합니다.

```bash
npx codemod@latest webpack/v5/migration-recipe
```

이렇게 하면 [Codemod 레지스트리](https://codemod.com/registry)에서 다음 codemod가 실행됩니다.
...
이러한 각 codemod는 webpack v5 마이그레이션 가이드에 나열된 변경 사항을 자동화합니다. 사용 가능한 webpack v5 codemod의 전체 목록은 [Codemod 레지스트리](https://go.codemod.com/webpack-v5)를 참고하세요.
...
- `optimization.hashedModuleIds: true` → `optimization.moduleIds: 'deterministic'` (`'hashed'`도 여전히 동작하지만, 더 이상 사용되지 않으며 경고를 기록합니다)
...
- `Compilation.entries` (엔트리 모듈의 배열) → `Compilation.entries`는 이제 `Map<string, EntryData>`입니다. `compilation.entries.get(name).dependencies`를 통해 엔트리의 의존성을 읽을 수 있습니다.
...
T> 설정 옵션의 자세한 변경 사항은 [여기](/blog/2020-10-10-webpack-5-release/#changes-to-the-configuration)에서 확인하세요.
...
만약 "Webpack 4 및 플러그인/로더 업그레이드" 단계에서 일부 플러그인/로더를 최신 버전으로 업그레이드하지 못했다면, 지금이라도 업그레이드하는 것을 잊지 마세요.
...
- `target`이 함수로 설정되어 있으면 `false`로 업데이트하고 `plugins` option 내에서 해당 함수를 적용하세요. 아래 예를 참고하세요.
  ...
  > **참고**: 이 변경 사항을 위한 codemod:
  >
  > ```bash
  > npx codemod webpack/v5/set-target-to-false-and-update-plugins
  > ```
  >
  > ([여기](https://codemod.com/registry/webpack-v5-set-target-to-false-and-update-plugins)에서 registry를 확인하세요.)
...
- output.library 또는 output.libraryTarget이 정의된 경우 속성 이름을 변경합니다 (output.libraryTarget -> output.library.type, output.library -> output.library.name). 예를 들면 다음과 같습니다.
  ...
  > ```
  >
  > ([여기](https://codemod.com/registry/webpack-v5-migrate-library-target-to-library-object)에서 registry를 확인하세요.)

import를 통해 WebAssembly를 사용하는 경우 다음 두 단계 프로세스를 따라야 합니다.

- webpack 4에서와 동일한 동작을 얻으려면 `experiments.syncWebAssembly: true`를 설정하여 더 이상 사용되지 않는 사양을 활성화합니다.
- webpack 5로 성공적으로 마이그레이션 한 후, `experiments.syncWebAssembly`를 다시 제거하세요. webpack 5.109.0부터 `experiments.asyncWebAssembly`의 기본값은 `'auto'`로, `syncWebAssembly`가 설정되거나 `module.rules` 규칙이 `.wasm` 파일과 일치하지 않는 한 최신 WASM 통합 사양을 활성화합니다. 이전 webpack 5 버전에서는 `experiments: { asyncWebAssembly: true }`를 명시적으로 설정하세요.
...
> ```
>
> (자세한 내용은 [레지스트리 링크](https://codemod.com/registry/webpack-v5-json-imports-to-default-imports)를 참고하세요.)
...
- Level 5: **Runtime Errors**.
  ...
  - 정의되지 않은 속성을 읽을 수 없습니다('call' 읽기)
    - 런타임 중에 이 오류가 발생하면 [ModuleConcatenationPlugin](https://webpack.js.org/plugins/module-concatenation-plugin/)과 관련이 있을 수 있습니다. 해당 플러그인을 사용 중인지 확인하고, 설정 파일의 `plugins` 섹션에 플러그인을 포함시켰으며 해당 설정이 `production` 모드로 설정되어 있는지 확인하세요. 플러그인 목록에서 해당 플러그인(예: `new webpack.optimize.ModuleConcatenationPlugin()`)을 제거하세요. webpack 5에서는 해당 플러그인이 프로덕션 모드에서 기본적으로 활성화되어 있으며, 두 번 포함될 수 있습니다.
    - 일반적으로 각 플러그인을 비활성화하고 빌드를 테스트하는 것은 문제의 원인을 파악하는 좋은 방법입니다.
    - 자세한 내용은 [이 문제](https://github.com/webpack/webpack/discussions/15369#discussioncomment-13420496)를 참고하세요.
...
[Pull Request](https://github.com/webpack/webpack.js.org/edit/main/src/content/migrate/5.mdx)를 만들어 이 가이드를 사용하는 다음 사람을 도와주세요.
...
- JSON5 대신 쿼리 문자열로 JSON을 지원합니다. `?{arg:true}` → `?{"arg":true}`. JSON5를 사용하는 것은 해당 로더의 문서에서 JSON을 위해 더 이상 사용되지 않는 것으로 간주하고 문서화되어야 합니다.
...
- 새로운 `this.getOptions` 메서드에 대해 스키마 인수는 선택 사항이지만 로더 옵션에 대한 스키마 유효성 검사를 추가하는 것이 좋습니다. 스키마의 `title` 필드는 유효성 검사 오류 메시지를 사용자 정의하는 데 사용할 수 있습니다. 예를 들면 `"title": "My Loader ooooptions"`는 다음과 같은 방식으로 오류를 표시합니다. `Invalid ooooptions object. My Loader has been initialized using an ooooptions object that does not match the API schema. - ooooptions.foo.bar.baz should be a string.`
