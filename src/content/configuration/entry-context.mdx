...
description: webpack의 Entry and Context 설정과 사용 방법을 설명합니다.
sort: 4
contributors:
  - sokra
  - skipjack
...
export default {
  ...
  entry: {
    ...
    personal: {
      ...
      worker: true, // worker 엔트리로 표시, webpack 5.108.0부터 사용 가능
    },
  },
};
```
...
`html` 옵션 <Badge text="5.108.0+" />은 이 엔트리 포인트의 JS와 CSS 출력 청크가 주입된 HTML 파일을 생성합니다. [`output.html`](/configuration/output/#outputhtml)과 같은 값을 받으며, webpack 5.110.0부터는 이 엔트리의 설정을 옵션별로 재정의하는 객체도 받습니다. 따라서 특정 엔트리만 페이지 생성을 제외하거나 title만 변경할 수 있습니다.
...
T> [`output.html.inline`](/configuration/output/#outputhtmlinline)은 생성된 페이지마다 한 번 리졸브되며 `output.html`에서만 설정할 수 있습니다.

T> webpack 5.108.0에 추가된 `worker` 플래그는 엔트리를 worker로 표시하여 출력 파일이 일반 청크 파일 이름 대신 [`output.workerChunkFilename`](/configuration/output/#outputworkerchunkfilename)을 사용하도록 합니다. webpack은 `new Worker(new URL(...))`에서 생성하는 엔트리에 이 값을 자동으로 설정하므로 직접 설정하는 경우는 드뭅니다.
...
경우에 따라서는 구조화 및 유지 관리 용이성을 위해 진입점을 여러 폴더로 분류하는 것이 좋을 수 있습니다. 다음은 여러 폴더에 위치한 여러 진입점을 정의하는 방법입니다.
...
`optimization.runtimeChunk`를 사용하면 지정되지 않은 엔트리포인트에 대해 전역적으로 설정할 수 있습니다.
