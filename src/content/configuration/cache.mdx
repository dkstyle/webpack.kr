---
title: Cache
description: webpack의 Cache 설정과 사용 방법을 설명합니다.
sort: 12
contributors:
  - snitin315
  - chenxsan
...
빌드 속도를 향상시키기 위해 생성된 webpack 모듈 및 청크를 캐시합니다. `cache: true`는 `cache: { type: 'memory' }`의 별칭입니다. 캐싱을 비활성화하려면 `false`를 전달하세요.

`cache`의 기본값은 [`mode`](/configuration/mode/)에 따라 다릅니다.
...
`cache.allowCollectingMemory`의 기본값은 [`mode`](/configuration/mode/)에 따라 다릅니다.
...
T> `'zstd'`(Zstandard)는 webpack 5.109.0부터 사용할 수 있으며, 내장 `node:zlib` zstd 바인딩을 제공하는 Node.js 22.15.0 이상이 필요합니다.

`cache.compression` 옵션은 [`cache.type`](#cachetype)이 `'filesystem'`으로 설정된 경우에만 사용할 수 있습니다.
...
`cache.managedPaths`는 설정 스키마에서 계속 허용(`type: 'filesystem'` 하에서만)되지만 무시됩니다. webpack은 설정을 정규화하는 동안 이 값을 삭제하고 패키지 관리자가 관리하는 경로에 실제로 적용되는 옵션인 [`snapshot.managedPaths`](/configuration/other-options/#managedpaths)만 읽습니다.
...
`cache.maxMemoryGenerations`의 기본값은 [`mode`](/configuration/mode/)에 따라 달라집니다.
...
`cache` 타입을 메모리 또는 파일 시스템으로 설정합니다. `memory` 옵션은 간단하며 webpack에 캐시를 메모리에 저장하도록 지시하고 [`cache.maxGenerations`](#cachemaxgenerations)와 [`cache.cacheUnaffected`](#cachecacheunaffected) 옵션만 허용합니다.
