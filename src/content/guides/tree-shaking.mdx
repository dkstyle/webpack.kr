---
title: Tree Shaking
description: webpack에서 Tree Shaking을 사용하는 방법을 설명합니다.
sort: 16
contributors:
  - simon04
  - zacanger
...
T> 가져온 모든 파일은 tree shaking의 대상이 됩니다. side effect만을 위해 가져온 CSS 파일이 프로덕션 모드에서 의도치 않게 제거되지 않도록 side effect 목록에 추가해야 합니다.
...
T> webpack 5.108.0부터 annotation이 모듈 경계를 넘어 전파되므로, import하는 모듈에서 사용되지 않는 호출도 트리 셰이킹됩니다. (5.108.0 이전에는 선언된 모듈 내에서만 효과가 있었습니다.)

T> 소스를 편집하여 annotation을 추가할 수 없는 경우(예: 의존성에서 가져온 함수), 설정에서 [`module.parser.javascript.pureFunctions`](/configuration/module/#moduleparserjavascriptpurefunctions)를 사용하여 이름을 사이드 이펙트 없음으로 표시할 수 있습니다(5.108.0+).
...
T> `--optimization-minimize` 플래그를 사용하여 `TerserPlugin`도 활성화할 수 있습니다.
