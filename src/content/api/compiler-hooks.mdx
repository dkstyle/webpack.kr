...
description: Compiler Hooks API의 기능과 사용 방법을 설명합니다.
group: Plugins
sort: 9
contributors:
  - rishantagarwal
...
플러그인 등록에 사용되는 [tapable](https://github.com/webpack/tapable) 훅(`SyncHook`, `AsyncSeriesHook` 등)의 `hooks` 객체를 노출합니다. 대부분의 사용자용 플러그인은 먼저 `Compiler`에 등록됩니다.
...
Webpack 설정, 플러그인 및 로더에 대한 유효성 검사 작업을 등록하기 위해 호출됩니다. 플러그인 개발자는 이 훅을 활용하여 `compiler.validate(...)`를 사용하여 webpack의 내장 유효성 검사 흐름에 참여할 수 있습니다.

W> 해당 훅은 항상 사용 가능합니다. `compiler.validate(...)`는 `validate: false`로 설정된 경우 유효성 검사를 건너뜁니다. `experiments.futureDefaults`를 사용하면 webpack은 개발 환경에서는 기본적으로 유효성 검사를 활성화하고 프로덕션 환경에서는 기본적으로 비활성화합니다.
...
컴파일이 시작되기 전에 이전 빌드의 [레코드](/configuration/other-options/#recordspath)를 읽기 위해 호출됩니다. 레코드 경로가 설정된 경우에만 실행됩니다.
...
새 컴파일이 생성되기 전인 `beforeCompile` 바로 뒤에 호출됩니다. 이 훅은 자식 컴파일러에 복사되지 _않습니다._
...
컴파일을 초기화하는 동안 `compilation` 이벤트를 생성하기 직전에 실행됩니다. 이 훅은 자식 컴파일러에 복사되지 _않습니다._
...
컴파일을 완료하기 전에 실행됩니다. 이 훅은 자식 컴파일러에 복사되지 _않습니다._
...
모든 엔트리와 그 디펜던시가 컴파일에 추가된 후, 완료되기 전에 [`make`](#make) 다음에 호출됩니다. 빌드에 이미 포함된 내용에 의존하는 모듈을 추가하는 데 사용하세요.
...
애셋을 출력 디렉터리로 방출하기 직전에 실행됩니다. 이 훅은 자식 컴파일러에 복사되지 _않습니다._

- 콜백 파라미터: `compilation`

W> **여기서 애셋을 추가하거나 변경하지 마세요.** `emit`은 컴파일이 봉인된 후에 실행되므로 모든 [`processAssets`](/api/compilation-hooks/#processassets) 스테이지가 이미 지나갑니다. 여기서 추가된 애셋은 최소화나 압축이 되지 않고, 소스맵도 없으며, 다른 플러그인도 볼 수 없습니다. Webpack은 이 방식의 쓰기를 더 이상 사용되지 않는 것으로 보고합니다(`DEP_WEBPACK_COMPILATION_ASSETS`). 대신 적절한 [스테이지](/api/compilation-hooks/#choosing-a-stage)와 함께 `processAssets`를 사용하세요. 여기서 애셋을 읽는 것은 괜찮습니다.

### afterEmit

`AsyncSeriesHook`

출력 디렉터리에 애셋을 내보낸 후 호출됩니다. 이 훅은 자식 컴파일러에 복사되지 _않습니다._
...
애셋이 방출된 후 이 빌드의 [레코드](/configuration/other-options/#recordspath)를 기록하기 위해 호출됩니다. [`readRecords`](#readrecords)와 마찬가지로 레코드 경로가 설정된 경우에만 실행됩니다.
...
컴파일이 완료되면 실행됩니다. 이 훅은 자식 컴파일러에 복사되지 _않습니다._
...
[`done`](#done)이 완료된 후, 실행의 콜백들이 정리되면 호출됩니다. 동기식이므로 빌드를 지연시킬 수 없습니다. 이로 인해 컴파일러를 멈추지 않아야 하는 보고 작업에 적합한 위치입니다.
...
감시중인 컴파일이 무효가 되었을 때 실행됩니다. 이 훅은 자식 컴파일러에 복사되지 _않습니다._
...
[`infrastructureLogging` 옵션](/configuration/infrastructureLogging)을 통해 설정에서 활성화된 경우 인프라 로깅을 사용할 수 있습니다.
