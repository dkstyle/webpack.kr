---
title: Plugin API
description: Plugin API의 기능과 사용 방법을 설명합니다.
group: Plugins
sort: 14
contributors:
  - thelarkinn
...
webpack의 많은 객체는 tapable 훅의 `hooks` 객체를 노출합니다. 각 훅은 `tap`을 노출하며, 타입에 따라 플러그인이 컴파일 전체에서 실행될 커스텀 빌드 단계를 삽입하는 데 사용할 수 있는 `tapAsync`와 `tapPromise` 메서드도 제공합니다.

자세한 내용은 [문서](https://github.com/webpack/tapable)를 참고하세요.
세 가지 `tap` 메소드와
이를 제공하는 훅을 이해하는 것은 중요합니다.
훅을 노출하는 객체(예: 컴파일러),
이들이 제공하는 훅, 각 훅의 타입(예: `SyncHook`)에 대해 알게 될 것입니다.
...
1. 컴파일 훅을 위한 모듈 범위 `WeakMap`을 만듭니다.
...
진행 상황을 보고하려면, 플러그인이 `ProgressPlugin.getReporter(compiler)`로 리포터를 가져와 일반 `tap`에서 호출합니다. `ProgressPlugin`이 활성화되어 있지 않으면 `getReporter`는 `undefined`를 반환합니다.
...
로깅 API는 webpack 4.37 릴리스부터 사용할 수 있습니다. [`stats 설정`](/configuration/infrastructureLogging)에서 `logging`이 활성화된 경우 또는 `infrastructure logging`이 활성화된 경우 플러그인은 각 로거 형식(stats, infrastructure)으로 메시지를 로깅 할 수 있습니다.
...
- 플러그인은 로깅 지원 여부를 확인하기 위해 특정한 폴백 로직 `compilation.getLogger ? compilation.getLogger('PluginName') : console`을 사용하여 `컴파일` 객체에서 `getLogger` 메소드를 지원하지 않는 이전 webpack 버전이 사용되는 경우에 대한 폴백을 제공 할 수 있습니다.
